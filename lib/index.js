...
function checkKindForLanguageConversion(kind) {
...
function getLanguages() {
  return targetStructorizer.api.doCall("GET", "/system/settings/langtags").value;
};
...
function grudStructorizer(baseUrl, options) {
  ...
  var Table = function () {
    ...
    _createClass(Table, [{
      ...
      value: function convertColumnToMultilanguage(tableName, columnName) {
        // const table = tables.find(tableName);
        ...
        // const {columns} = table.fetch();
        // const column = _.find(columns, {name: columnName});

        // if (!column) {
        //   console.warn(`Column not found in '${tableName}': '${columnName}'`);
        //   return;
        // }
        ...
        //   console.warn(`Language '${language}' not in '/system/settings/langtags'`);
        //   return;
        // }

        // const {ordering, kind, identifier, displayName, description} = column;
        // const ALLOWED_TYPES = ["shorttext", "text"];

        // if (!_.includes(ALLOWED_TYPES, kind)) {
        //   console.warn(`Column '${columnName}' must be of kind '${_.join(ALLOWED_TYPES, "' or '")}'`);
        //   return;
        // }

        // const columnIndex = _.findIndex(columns, {name: columnName});

        // targetStructorizer.api.doCall("POST", "/tables/" + table.tableId + "/columns/" + column.id, {
        //   name: columnName + "_old"
        // });

        // const {rows} = table.fetch(true);

        // const newColumnId = table.createColumn(
        ...
        //   const url = "/tables/" + table.tableId + "/columns/" + newColumnId + "/rows/" + rowId;
        ...
        //   const newValue = mapValueIntoLanguage(value, language || defaultLanguage);

        //   targetStructorizer.api.doCall("PATCH", url, newValue);

        //   targetStructorizer.api.doCall("POST", `${url}/annotations`, {
        //     langtags: languages,
        //     type: "flag",
        ...
        // targetStructorizer.api.doCall("DELETE", "/tables/" + table.tableId + "/columns/" + column.id);

        var language = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "de";
      }
    }]);
